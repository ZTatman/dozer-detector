...
import { Tensor3D, Tensor2D, Tensor1D, Tensor4D } from '@tensorflow/tfjs';
...
export function getGaussianKernel(size = 5): Tensor4D {
    return tf.tidy(() => {
        ...
        var d3 = tf.stack([d2, d2, d2], 2)
        return tf.expandDims(d3, 3) as Tensor4D
    })
}

export function blur(image: Tensor3D, kernel: Tensor4D): Tensor3D {
    return tf.tidy(() => {
        return tf.depthwiseConv2d(image.toFloat(), kernel, 1, "valid")
    })
}
